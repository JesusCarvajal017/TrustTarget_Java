...
async function Data(){
    ...
    data.forEach((entity,i ) => {
        ...
        <tr>
            <td>${i + 1}</td>
            <td>${entity.nation_name}</td>
            <td><i class="fas fa-edit m-2" onclick="Updateform(${entity.nation_id})" ></i><i class="fas fa-trash-alt m-2" onclick="Deleteform(${entity.nation_id})"></i> </td>
            <!-- fas fa-plus -->
        </tr>`;
    });
...
formInsert.addEventListener('submit', async (e) => {
    ...
    let resgister = await pais.create(data);
...
window.Updateform = async (id) => {
    let entity = await pais.getById(id);
    ...
    formUpdate.addEventListener('submit', async (e) => {
        ...
        let updateform = await pais.update(data);
...
btnDelete.addEventListener('click', async () => {
    if(currentDeleteId !== null){
        let deleteform = await pais.delete(currentDeleteId);
...
window.Deleteform = async (id) => {
    currentDeleteId = id;
    let htmlView = "";
    let data = await pais.getById(id);
