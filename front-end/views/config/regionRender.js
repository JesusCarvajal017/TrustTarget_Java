...
async function Data(){
    ...
    data.forEach((entity,i ) => {
        ...
        <tr>
            <td>${i + 1}</td>
            <td>${entity.reg_name}</td>
            <td>${entity.reg_code}</td>
            <td><i class="fas fa-edit m-2" onclick="Updateform(${entity.reg_id})" ></i><i class="fas fa-trash-alt m-2" onclick="Deleteform(${entity.reg_id})"></i> </td>
            <!-- fas fa-plus -->
        </tr>`;
    });
...
formInsert.addEventListener('submit', async (e) => {
    ...
    let resgister = await region.create(data);
...
window.Updateform = async (id) => {
    let entity = await region.getById(id);

    let dataform = {
        reg_name: entity.reg_name,
        reg_code: entity.reg_code,
    }
    ...
    formUpdate.addEventListener('submit', async (e) => {
        ...
        let updateform = await region.update(data);
...
btnDelete.addEventListener('click', async () => {
    if(currentDeleteId !== null){
        let deleteform = await region.delete(currentDeleteId);
...
window.Deleteform = async (id) => {
    currentDeleteId = id;
    let htmlView = "";
    let data = await region.getById(id);
